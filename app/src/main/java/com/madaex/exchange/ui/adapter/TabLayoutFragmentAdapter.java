...
public class TabLayoutFragmentAdapter extends FragmentPagerAdapter {
     ...
     * @return
     */
    public View getTabView(int position,int crruent) {
        ...
        if (0 == position&&0 == crruent) {//the default color of item home is green
            mTabText.setTextColor(ContextCompat.getColor(mContext, R.color.title_color));
            mTabIcon.setImageResource(R.mipmap.icon_market_select);
        }if (4 == position&&4 == crruent) {//the default color of item home is green
            mTabText.setTextColor(ContextCompat.getColor(mContext, R.color.title_color));
            mTabIcon.setImageResource(R.mipmap.icon_mine_select);
        }
        return view;
    }
}
