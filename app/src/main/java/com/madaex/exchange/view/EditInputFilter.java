...
public class EditInputFilter implements InputFilter {
     ...
     * 最大数字
     */
    public  int MAX_VALUE = 500000;

    /**
     * 小数点后的数字的位数
     */
    public  int POINTER_LENGTH = 6;

    private static final String POINTER = ".";

    Pattern p;

    public EditInputFilter(int valus) {
        MAX_VALUE = valus;
        //用于匹配输入的是0-9  .  这几个数字和字符
        p = Pattern.compile("([0-9]|\\.)*");
    }
    public EditInputFilter(int valus,int count) {
        MAX_VALUE = valus;
        POINTER_LENGTH = count;
        //用于匹配输入的是0-9  .  这几个数字和字符
        p = Pattern.compile("([0-9]|\\.)*");
    }
    /**
