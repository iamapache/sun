...
public class KLineDataManage {
        ...
        if (object != null) {
            ...
            if (jsonObject != null) {
                ...
                if (data != null) {
                    ...
                    List<Double> ma5List = calculateMA(5, kDatas);
                    List<Double> ma10List = calculateMA(10, kDatas);
                    List<Double> ma30List = calculateMA(30, kDatas);
                    for (int i = 0; i < kDatas.size(); i++) {
                        KLineDataModel hisData = kDatas.get(i);
                        ...
                        line20Entries.add(new Entry(i, i + offSet, (float) getKLineDatas().get(i).getMa30()));
                    }
                    candleDataSet = setACandle(candleEntries);
                    bollCandleDataSet = setBOLLCandle(candleEntries);
                    volumeDataSet = setABar(barEntries, mContext.getString(R.string.transnumber));
     ...
     * 计算MA
     */
    public static List<Double> calculateMA(int dayCount, List<KLineDataModel> data) {
        dayCount--;
